...
def cost(
    ...
    if file1z != None:
        similarity = shape_similarity3D(
            shape1, shape2, checkRotation=False
        )  # * checkRotation=True allows the method to don't take into account the orientation of the movements #* checkRotation=True allows the method to don't take into account the orientation of the movements
        if plot == True:
            plotShape3D(shape1, shape2, similarity)
        return similarity, shape1, shape2
    elif file1z == None:
        similarity = shape_similarity(
            shape1, shape2, checkRotation=True
        )  # * checkRotation=True allows the method to don't take into account the orientation of the movements
        if plot == True:
...
def plotShape(shape1, shape2, similarity):
    ...
    if similarity > 0.5:
        plt.title(
            f"Similarity is: {similarity:.2f}, considered as same primitives",
            fontsize=14,
            fontweight="bold",
            color="green",
        )
        ...
        plt.title(
            f"Similarity is: {similarity:.2f}, considered as different primitives",
            fontsize=14,
            fontweight="bold",
            color="red",
        )
