...
class Screen_Capture(object):
    ...
    def capture(self):
        ...
        st_rect = self.smooth_text.get_rect(
            center=(
                self.smooth_button_coords[0] + (self.smooth_button_coords[2] // 2),
                self.smooth_button_coords[1] + (self.smooth_button_coords[3] // 2),
            )
        )
        raw_rect = self.raw_text.get_rect(
            center=(
                self.raw_button_coords[0] + (self.raw_button_coords[2] // 2),
                self.raw_button_coords[1] + (self.raw_button_coords[3] // 2),
            )
        )
        sv_rect = self.save_text.get_rect(
            center=(
                self.save_button_coords[0] + (self.save_button_coords[2] // 2),
                self.save_button_coords[1] + (self.save_button_coords[3] // 2),
            )
        )
        ld_rect = self.load_text.get_rect(
            center=(
                self.load_button_coords[0] + (self.load_button_coords[2] // 2),
                self.load_button_coords[1] + (self.load_button_coords[3] // 2),
            )
        )
        qt_rect = self.quit_text.get_rect(
            center=(
                self.quit_button_coords[0] + (self.quit_button_coords[2] // 2),
                self.quit_button_coords[1] + (self.quit_button_coords[3] // 2),
            )
        )
        sel_rect = self.select_text.get_rect(
            center=(
                self.selector_text_coords[0] + (self.selector_text_coords[2] // 2),
                self.selector_text_coords[1] + (self.selector_text_coords[3] // 2),
            )
        )
        ...
        screen.blit(
            self.select_text,
            (self.selector_text_coords[0], self.selector_text_coords[1] + 40),
        )
        ...
        while True:
                ...
                for i in self.selected_demos:
                    for j in range(len(self.smoothed_demos[i][0])):
                        ...
                        pygame.draw.circle(
                            ...
                            pd_color,
                            (
                                int(self.smoothed_demos[i][1][j]),
                                int(self.smoothed_demos[i][2][j]),
                            ),
                            3,
                        )
            else:
                for i in self.selected_demos:
                    for j in range(len(self.demos[i][0])):
                        ...
                        pygame.draw.circle(
                            screen,
                            pd_color,
                            (self.demos[i][1][j], self.demos[i][2][j]),
                            3,
                        )
            ...
            while recording == False:
                pygame.draw.rect(screen, self.demo_bg_color, self.demo_window_coords)
                pygame.draw.rect(screen, color, self.selector_text_coords)
                self.selected_text = self.sf.render(
                    str(self.selected_demos) + text, True, self.demo_color
                )
                ...
                if pygame.mouse.get_pressed() == (1, 0, 0):
                    ...
                    if inbounds(x, y, self.load_button_coords):
                        ...
                        if len(filename) > 2:
                            ...
                            read_demos = (
                                np.array(list(smooth.keys())).astype(int).tolist()
                            )
                            fp.close()
                            self.demos = []
                            self.smoothed_demos = []
                    ...
                    if inbounds(x, y, self.selector_text_coords):
                        ...
                        print(
                            'Please enter the demos you wish to have currently selected (example "0 2") and press enter'
                        )
                        active = True
                    ...
                    if event.type == pygame.KEYDOWN:
                        if active:
                            if event.key == pygame.K_RETURN:
                                print(text)
                                self.selected_demos = (
                                    np.array(text.split()).astype(int).tolist()
                                )
                                text = ""
